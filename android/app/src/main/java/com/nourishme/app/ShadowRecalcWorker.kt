...
class ShadowRecalcWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    override suspend fun doWork(): Result {
        return try {
            ...
            val endpoint = prefs.getString(
                "shadow:endpoints:recalc",
                "https://nourish-me.vercel.app/api/shadow/state/recalc?cron=1"
            ) ?: "https://nourish-me.vercel.app/api/shadow/state/recalc?cron=1"
            httpGet(endpoint)
            Result.success()
        } catch (_: Throwable) {
