...
class ShadowMorningCheckInWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    override suspend fun doWork(): Result {
        return try {
            ...
            val endpoint = prefs.getString(
                "shadow:endpoints:morning",
                "https://nourish-me.vercel.app/api/shadow/state/today?cron=1"
            ) ?: "https://nourish-me.vercel.app/api/shadow/state/today?cron=1"
            httpGet(endpoint)
            Result.success()
        } catch (_: Throwable) {
