...
// Phase 5: Daily event generator — use shadow_task_instances as events for today
export async function POST() {
  try {
    ...
    // Load shadow_tasks joined to tasks for ordering
    ...
    // Insert missing candidates (skip already existing)
    ...
    const { data: events } = await supabase
      .from('shadow_task_instances')
      .select('id, shadow_task_id, planned_start_at, planned_end_at, planned_date_local, status, progress')
      .eq('planned_date_local', dayStr);

    return NextResponse.json({ ok: true, planned_date_local: dayStr, count: events?.length || 0, events });
  } catch (e: any) {
