...
export async function POST(req: NextRequest) {
  ...
  const upsert = {
    user_id: userId,
    // timezone now supported here so all clients can use /api/preferences
    ...
    activity_level:
      body.activity_level ?? existing?.activity_level ?? "sedentary",
    ...
    carbs_goal_grams:
      body.carbs_goal_grams ?? existing?.carbs_goal_grams ?? null,
    // New flag: whether onboarding has been seen
    has_seen_onboarding:
      body.has_seen_onboarding ?? existing?.has_seen_onboarding ?? false,
    updated_at: new Date().toISOString(),
    dietary_restrictions:
      body.dietary_restrictions ?? existing?.dietary_restrictions ?? null,
  };
