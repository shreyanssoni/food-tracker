...
export async function GET() {
  try {
    ...
    const summaries: Record<string, { totalWeeks: number; successWeeks: number }> = {};
    for (const g of goals || []) {
      const { data: rows } = await supabase.rpc('fn_goal_weekly_success', { p_goal_id: g.id });
      ...
      startOfThisWeek.setDate(startOfThisWeek.getDate() - startOfThisWeek.getDay()); // Sunday as week start
...
export async function POST(req: Request) {
  try {
    ...
    const { title, description, deadline, templates = [], collectible } = body || {};
    if (!title || !deadline || !Array.isArray(templates) || templates.length === 0) {
      return NextResponse.json({ error: 'Missing title, deadline, or templates' }, { status: 400 });
    }
    ...
    if ((activeGoals?.length || 0) >= MAX_ACTIVE_GOALS) {
      return NextResponse.json({ error: `You can have at most ${MAX_ACTIVE_GOALS} active goals.` }, { status: 400 });
    }

    if (templates.length > MAX_TEMPLATES_PER_GOAL) {
      return NextResponse.json({ error: `Limit ${MAX_TEMPLATES_PER_GOAL} templates per goal.` }, { status: 400 });
    }
    ...
    for (const t of templates) {
      ...
      if (t.ep_value > 200) {
        return NextResponse.json({ error: 'EP value cannot exceed 200' }, { status: 400 });
      }
      if (typeof t.times_per_period !== 'number' || t.times_per_period < 1) {
        return NextResponse.json({ error: 'times_per_period must be at least 1' }, { status: 400 });
      }
      if (t.times_per_period > MAX_TIMES_PER_PERIOD) {
        return NextResponse.json({ error: `times_per_period cannot exceed ${MAX_TIMES_PER_PERIOD}` }, { status: 400 });
      }
    }
    ...
    for (const t of templates) {
      if ((t.frequency || 'weekly') === 'weekly') {
        weeklyEpFromTemplates += (Math.min(200, t.ep_value ?? 10)) * (Math.min(MAX_TIMES_PER_PERIOD, t.times_per_period ?? 1));
      }
    }
    if (weeklyEpFromTemplates > WEEKLY_EP_CAP) {
      return NextResponse.json({ error: `Total weekly EP across templates capped at ${WEEKLY_EP_CAP}. Reduce EP or frequency.` }, { status: 400 });
    }
    ...
    const startDateStr = new Date(startOfToday.getFullYear(), startOfToday.getMonth(), startOfToday.getDate()).toISOString().slice(0,10);
    const { data: goal, error: gErr } = await supabase
      .from('goals')
      .insert({ user_id: user.id, title, description, deadline, start_date: startDateStr })
      .select('*')
      .single();
    if (gErr) throw gErr;
    ...
    for (const t of tpl || []) {
      ...
      const { error: linkErr } = await supabase
        .from('goal_tasks')
        .insert({ goal_id: goal.id, template_id: t.id || t, task_id: taskRow.id });
    ...
    if (collectible?.name) {
      ...
      let weeklyEp = 0;
      for (const t of tpl || []) {
        if (t.frequency === 'weekly') weeklyEp += (t.ep_value ?? 10) * (t.times_per_period ?? 1);
      }
      ...
      const [{ error: gcErr }, { error: reqErr }] = await Promise.all([
        supabase.from('goal_collectibles').insert({ goal_id: goal.id, collectible_id: col.id }),
