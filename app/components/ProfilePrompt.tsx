...
export default function ProfilePrompt() {
      ...
      .then((d) => {
        ...
        if (p) {
          setForm((prev) => ({
            ...
            dietary_restrictions:
              p.dietary_restrictions || prev.dietary_restrictions,
          }));
        }
      })
      .catch(() => {});
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-xl bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-5 shadow-lg">
        <h2 className="text-lg font-semibold">Tell us about you</h2>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
          We use this to personalize your daily calorie and macro targets.
        </p>
        <form onSubmit={onSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <label className="text-sm">
              Height (cm)
              <input
                className="mt-1 w-full border border-gray-200 dark:border-gray-800 rounded-md px-2 py-1 bg-white dark:bg-gray-900"
            ...
            <label className="text-sm">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, activity_level: e.target.value as any })
                }
              >
            ...
            <label className="text-sm">
              ...
              <select
                className="mt-1 w-full border border-gray-200 dark:border-gray-800 rounded-md px-2 py-1 bg-white dark:bg-gray-900"
                value={form.workout_level}
                onChange={(e) =>
                  setForm({ ...form, workout_level: e.target.value as any })
                }
              >
          ...
          <div className="flex justify-end gap-2 pt-2">
            {error && (
              <div className="mr-auto text-xs text-red-600 dark:text-red-400">
                {error}
              </div>
            )}
            <button
              type="submit"
              disabled={loading}
              className="px-3 py-1 rounded-md bg-emerald-600 text-white disabled:opacity-60"
            >
              {loading ? "Saving…" : "Save"}
            </button>
          </div>
